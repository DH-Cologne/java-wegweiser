...
Beim Programmieren sieht das dann etwa so aus, dass eine Methode sich selbst aufruft. Vorsichtig sollte man natürlich mit potenziellen Endlos-Schleifen sein - es muss immer irgendwann einen Zustand geben, in dem die Methode "fertig" ist.

Ein ganz klassisches Beispiel ist eine Methode zur Errechnung der [Fakultät](https://de.wikipedia.org/wiki/Fakult%C3%A4t_(Mathematik)) einer Zahl. Mathematisch definiert sich die Fakultät einer (natürlichen) Zahl als das Produkt _aller_ natürlichen Zahlen von `1` bis `n` (also der Zahl selbst):
